...
namespace ArchiMetrics.CodeReview.Rules.Trivia
{
	...
	internal abstract class CommentLanguageRuleBase : TriviaEvaluationBase
	{
		...
		protected override EvaluationResult EvaluateImpl(SyntaxTrivia node)
		{
			...
			// A comment can reduce to nothing once separators, XML tags and numbers are stripped.
			// Divider lines (// ------), bare markers (// <auto-generated>) and blank // lines all
			// end up here. There is no prose to judge, so there is nothing to report: bailing out
			// early keeps the rule focused on real comments instead of drowning callers in noise.
			// Without this guard the threshold below reads 0 >= 0, which is true, so every such
			// comment would be flagged as suspicious.
